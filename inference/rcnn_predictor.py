...
def extend_label(image):
    # init
    img_name = image['file_name'].split('/')[-1].split('.')[0]
    ori_height = image['height']
    ori_width = image['width']
    old_height, old_width = image['image'].shape[1], image['image'].shape[2]
    ratio_height, ratio_width = old_height / ori_height, old_width / ori_height
    
    # load pseudo label
    label_dir = '/data/lhm/FreeSOLO-main/demo/freesolo_output'
    ...
    for label in labels:
        label = label.split(' ')
        tmp = []
        label[0] = float(label[0]) * ratio_width
        label[1] = float(label[1]) * ratio_height
        label[2] = float(label[2]) * ratio_width
        label[3] = float(label[3]) * ratio_height
        tmp.append(label[0])
        tmp.append(label[1])
        tmp.append(label[2])
        tmp.append(label[3])
...
def update_proposals(proposals, pseudo_labels, height, width):
    label_num = len(pseudo_labels)
    objectness = proposals[0].objectness_logits[0]

    current_device = proposals[0].proposal_boxes.device
    pseudo_labels = torch.tensor(pseudo_labels).to(current_device)
    # pseudo_labels = Boxes(pseudo_labels)

    start = 20
    tmp_labels = proposals[0].proposal_boxes.tensor
    tmp_labels[start: start + label_num] = pseudo_labels
    merge_Boxes = Boxes(tmp_labels)
    # merge_Boxes = Boxes.cat([proposals[0].proposal_boxes, pseudo_labels])
    
    new_objectness = torch.tensor([objectness for i in range(label_num)]).to(current_device)
    tmp_objectness = proposals[0].objectness_logits.detach()

    tmp_objectness[start:start + label_num] = new_objectness
    merge_objectness = tmp_objectness
    # merge_objectness = torch.cat((proposals[0].objectness_logits, new_objectness))
    
    
    new_proposals = Instances((height, width))
    new_proposals.objectness_logits = merge_objectness
    new_proposals.proposal_boxes = merge_Boxes
...
class GeneralizedRcnnPlainPredictor(ProbabilisticPredictor):
    ...
    def generalized_rcnn_probabilistic_inference(self,
        ...
        if is_epistemic:
            ...
            ####
            raw_output = dict()
            # pseudo_labels = extend_label(input_im[0])
            ...
            old_height, old_width = input_im[0]['image'].shape[1], input_im[0]['image'].shape[2]
            # proposals = update_proposals(proposals, pseudo_labels, old_height, old_width)
            ...
            """ vis coordinates"""
            # numpy_images = images.tensor.cpu().numpy()
            # numpy_images = np.squeeze(numpy_images)
            # numpy_images = np.transpose(numpy_images, (1, 2, 0)).copy() 
            # for label in pseudo_labels:
            #     cv2.rectangle(numpy_images, (int(label[0]), int(label[1])), (int(label[2]), int(label[3])), (255, 0, 255))
            # cv2.imwrite('./numpy_images.png', numpy_images)
           
          
            
            # box_features_ood = self.model.roi_heads.extract_feature_mix(features, [x.proposal_boxes for x in proposals])
            # box_features_ood: [N, 1024]
