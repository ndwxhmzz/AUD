...
def build_predictor(cfg):
    ...
    if cfg.MODEL.META_ARCHITECTURE == 'ProbabilisticRetinaNet':
        ...
        return GeneralizedRcnnPlainPredictor(cfg)
    elif cfg.MODEL.META_ARCHITECTURE == 'GeneralizedRCNNLogisticGMM_AugFPN':
        return GeneralizedRcnnPlainPredictor(cfg)
    else:
        raise ValueError(
...
def OOD_detection_postprocessing(input_im,
    ...
    if not pretest:
        ...
        keep = remove_by_scores(outputs, 0.515, retain_larger=True)
        update(outputs, keep)
...
def mergeIDOOD_iou(ID_results, OOD_results, pretest):
    if pretest:
    ...
    ioc = LIoU(ID_boxes, OOD_boxes, 0)
    ioc_score, ioc_index = ioc.max(0)
  
    iop = LIoU(ID_boxes, OOD_boxes, 1)
    iop_score, iop_index = iop.max(0)
    
    OOD_maxiou_with_ID = iou.max(0)[0]
    OOD_index = torch.where(OOD_maxiou_with_ID < 1)
    
    # print(ioc)
    OOD_keep = []
      
    for i in range(len(OOD_boxes)):
        if ioc_score[i] < 0.8 and iop_score[i] < 0.8:
            OOD_keep.append(i)
        else:
            if ID_scores[ioc_index[i]] < 0.7 and ID_scores[iop_index[i]] < 0.7:
                OOD_keep.append(i)
    ...
    return Instances.cat([ID_results, OOD_results[OOD_keep]])
